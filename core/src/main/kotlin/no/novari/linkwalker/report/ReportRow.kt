...
data class ReportRow(
    ...
    @field:Schema(
        description = "Name of the `_links` relation on the source record. Null for `unknown-link`.",
        nullable = true,
    )
    val relationName: String? = null,
    @field:Schema(
        description = "Name of the inverse relation that should have pointed back. Set only for `missing-back-link-*`.",
        nullable = true,
    )
...
data class LatestReportSummary(
    ...
    val scanCompletedAt: Instant,
    @field:Schema(description = "Tenant org-id this report belongs to.", example = "agderfk-no")
    val orgId: String,
